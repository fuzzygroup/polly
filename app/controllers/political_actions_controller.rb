class PoliticalActionsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @political_action.save
        format.html { redirect_to @political_action, notice: "Political action was successfully created." }
  ...
  def update
    respond_to do |format|
      if @political_action.update(political_action_params)
        format.html { redirect_to @political_action, notice: "Political action was successfully updated." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to political_actions_path, status: :see_other, notice: "Political action was successfully destroyed." }
  ...
  private
    ...
    def set_political_action
      @political_action = PoliticalAction.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def political_action_params
      params.expect(political_action: [ :name, :active, :organization_id, :user_id, :details ])
    end
end
